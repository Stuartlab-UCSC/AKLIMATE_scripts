...
#****************************************************************************************************************
# This script continues the process of the "find_balance_accuracy_cohorts.R" script. Instead of looking at the 
#balance accuracies of each cohort however, we find the balance accuracy of the invidivual subcorts in each cohort 
#and individually plot each tumor types sub tumor types.
#****************************************************************************************************************
...
for (j in 1:length(tumor_type_list)) {
  ...
  if(tt == "coadread"){
    text(x-0.25, rep(-0.10, length.out=length(x)), labels = sub_cohort_names[1:4], srt = 60, cex = .8)
  }
  dev.off()
}
